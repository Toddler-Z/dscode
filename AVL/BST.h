...
protected:
    ...
    /**
     * @brief 找到以t为根的子树的最小节点，返回这个节点的指针，并将这个节点删去更新
     * 
     * @param t 子树的根节点指针
     * @return 最小节点的指针
     */
    ...
    void remove(const Comparable &x, BinaryNode * &t) {
        ...
        /// 进入以下这两个分支，都是说明找到了要删除的元素
            ...
            BinaryNode *minNode = detachMin(t->right);  // 找到最小节点的指针,将其浅复制给 minNode，并更新这个节点
            minNode->left=t->left;  // 将原来的左子树链接到 minNode 的左子树
            minNode->right=t->right;  // 将原来的右子树链接到 minNode 的右子树
            BinaryNode *oldNode = t; // 保存当前节点 t = minNode; 
            t = minNode;// 用 minNode 替换当前节点 delete oldNode; 
            delete oldNode;  // 删除当前节点，释放内存           
        } else {
            /// 有一个或没有子节点的情形是简单的
